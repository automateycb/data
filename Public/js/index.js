function initGithubCharts() {
    if (!githubChartsInited) { githubChartsInited = !0;
        var t = echarts.init($("#github-line-chart")[0]);
        t.setOption({ animationDuration: 1500, series: [{ name: "日均百度搜索指数", type: "line", smooth: !0, itemStyle: { normal: { color: gb.colors.white } }, areaStyle: { normal: { color: gb.colors.whiteLight } }, lineStyle: { normal: { width: 1 } }, data: gb.homeData, symbolSize: 0, markPoint: { itemStyle: { normal: { color: "#a9334c", label: { textStyle: { color: gb.colors.white }, formatter: function(t) {
                                    return t.name } } } }, symbolSize: function(t, e) {
                        return "v3.0.0" === e.name, 50 }, data: [{ name: "v3.0.0", value: "2016年1月", xAxis: "2016年1月", yAxis: 3200, itemStyle: { normal: { color: "#a9334c", label: { textStyle: { color: gb.colors.white } } } } }, { name: "v2.0.0", value: "2014年7月", xAxis: "2014年7月", yAxis: 675 }, { name: "v1.3.7", value: "2014年1月", xAxis: "2014年1月", yAxis: 0 }] } }], xAxis: [{ type: "category", splitLine: { show: !0, lineStyle: { type: "dashed", color: gb.colors.whiteLighter } }, axisLine: { show: !1 }, axisTick: { show: !1 }, data: [{ value: "2014年1月", textStyle: { align: "left" } }, "2014年2月", "2014年3月", "2014年4月", "2014年5月", "2014年6月", "2014年7月", "2014年8月", "2014年9月", "2014年10月", "2014年11月", "2014年12月", "2015年1月", "2015年2月", "2015年3月", "2015年4月", "2015年5月", "2015年6月", "2015年7月", "2015年8月", "2015年9月", "2015年10月", "2015年12月", "2016年1月", "2016年2月", { value: "2016年3月", textStyle: { align: "right" } }], boundaryGap: 0, axisLabel: { textStyle: { color: gb.colors.whiteLight } } }], yAxis: [{ type: "value", show: !1, max: 4e3 }], grid: { borderWidth: 0, x: 20, y: 100, x2: 20, y2: 40 }, tooltip: { trigger: "item", formatter: function(t) {
                    return "v" === t.name[0] ? t.value + " 发布 " + t.name + " 版本" : "2016年1月" === t.name[0] ? "ECharts 3.0 时代" : t.name + "，" + t.seriesName + "：" + t.value } } }), setTimeout(function() { t.setOption({ series: [{ animationDurationUpdate: 1e3, name: "日均百度搜索指数", symbolSize: 8 }] }) }, 1500), $(window).resize(function() { t.resize() }) } }

function initSizeCharts() {
    if (!sizeChartInitialized) { sizeChartInitialized = !0;
        var t = echarts.init($("#file-size-chart")[0]),
            e = ["-", { value: 0, itemStyle: { normal: { color: "#293c55" } } }, { value: 0, itemStyle: { normal: { color: "#a9334c" } } }, "-"];
        t.setOption({ animation: !1, grid: { left: -20, right: 20, top: 4, bottom: 30 }, textStyle: { fontSize: 14 }, xAxis: { axisLine: { lineStyle: { color: "#ccc" } }, axisLabel: { textStyle: { fontSize: 14, color: "#999" } }, axisTick: { show: !1 }, splitLine: { show: !1 }, data: ["", "ECharts 2", "ECharts 3", ""] }, yAxis: { min: 0, max: 400, splitNumber: 4, splitLine: { lineStyle: { color: "#eee" } }, axisLine: { show: !1 }, axisLabel: { show: !1 }, axisTick: { show: !1 } }, series: [{ type: "bar", barCategoryGap: "50%", label: { normal: { formatter: "{c} KB", show: !0, position: "top" } }, data: e }] });
        var i = new echarts.graphic.Group,
            a = new echarts.graphic.Group;
        t.getZr().add(i), t.getZr().add(a), i.animate().when(2e3, { position: [373, 373] }).during(function() { e[1].value = Math.round(i.position[0]), e[2].value = Math.round(a.position[1]), t.setOption({ series: [{ data: e }] }) }).start(), a.animate().when(1e3, { position: [172, 172] }).start(), $(window).on("resize", function() { t.resize() }) } }
...
function drawCanvasOne() {
    ...
    for (var r = 0; 3 > r; r++) {
        for (var s = [], l = 0; n + 1 >= l; l++) {
            ...
            s.push(m[0], m[1]) }
        var u = new echarts.graphic.Polyline({ shape: { points: s, smooth: .4 }, style: { stroke: "#fff", opacity: 1 / (r + 1), lineWidth: 1.2 / (r + 1) + .8 }, silent: !0, position: [-r * a / 8, 35 * -(r - .5)] }),
    ...
    $(window).on("resize", function() {
        ...
        var n = a / t,
            r = c / i;
        o.eachChild(function(t) { t.position[0] *= n, t.position[1] *= r, t.shape.points.forEach(function(t) { t[0] *= n, t[1] *= r }), t.dirty(!0) }) }) }

function drawLogo() {
    function t() { n.style.stroke = null, r.style.stroke = null;
    ...
    var e = document.getElementById("animate-logo"),
        ...
        n = echarts.graphic.makePath(c, { silent: !0 }, { x: -.5, y: -.5, width: 1, height: 1 }, "center");
    ...
    n.dashOffset = 0, n.animate().when(1500, { dashOffset: l }).during(function() {
        var t = n.dashOffset;
        n.setStyle({ lineDash: [t, l - t] }), r.setStyle({ lineDash: [t, l - t] }) }).done(function() { n.animateTo({ style: { fill: n.style.stroke } }, 1500, function() { n.setStyle({ fill: "#AA314D" }), n.animateTo({ rotation: .6 }, 800, "bounceOut") }), r.animateTo({ style: { fill: r.style.stroke } }, 1500) }).start();
    var h;
    $(window).resize(t) }! function() { $(".lowerie").length }();
var gb = { colors: { white: "#FFF", whiteLight: "rgba(255, 255, 255, 0.2)", whiteLighter: "rgba(255, 255, 255, 0.1)", primary: "#556fb5", primaryLight: "#889acb" }, homeData: [0, 0, 0, 0, 200, 449, 675, 723, 724, 655, 792, 846, 840, 927, 687, 1148, 1172, 1332, 1370, 1730, 1786, 1820, 1920, 3200, 2200, 2400] },
